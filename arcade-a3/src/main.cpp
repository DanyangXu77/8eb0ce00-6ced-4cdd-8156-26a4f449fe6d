...
void autonomous(void) {
  ...
  ee();
  drivef(1, true);
  Intake.spinFor(forward, 600, degrees, 300, rpm);
  ee_stop();
  drivef(1, false);
  ...
  eer();
  drivef(5, true);
  Intake.spinFor(forward, 600, degrees, 600, rpm);
...
void usercontrol(void) {
  ...
  BackLeft.setVelocity(600, rpm);
  BackRight.setVelocity(600, rpm);
  FrontLeft.setVelocity(600, rpm);
  FrontRight.setVelocity(600, rpm);
  MiddleLeft.setVelocity(600, rpm);
  MiddleRight.setVelocity(600, rpm);

  velocity_ = 160;

  // User control code here, inside the loop
  while (1) {
    a = Controller1.Axis1.position(percent);
